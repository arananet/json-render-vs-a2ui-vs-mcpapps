...
\texttt{ENFORCED}/\texttt{NOT\_EXPRESSIBLE} labels do not measure
collision prevention or impossibility of composition; separate
assertions check the two retained headline values. A2UI S2 checks labels
and the writer map, not Node-rendered text. The bridge tests instead
exercise calls and assert actual returned content or rejection plus
refusal records. None of these labels is an empirical score.
...
\href{./evidence/browser-s2-fixed-order-T1RMZk03/playwright-ipv4.json}{raw
...
\href{./evidence/browser-s2-fixed-order-T1RMZk03/playwright-ipv4.json}{current
JSON report},
\href{./evidence/browser-s2-fixed-order-T1RMZk03/source-before.json}{source
snapshots},
\href{./evidence/browser-s2-fixed-order-T1RMZk03/built-before.json}{build
hashes}, and
\href{./evidence/browser-s2-fixed-order-T1RMZk03/ipv4-post-run-screenshots.sha256}{capture
...
The suite does not comprehensively test adversarial inputs, identity
forgery, transport authentication, CSP, cross-server policy,
authorization revocation, or arbitrary concurrent schedules. No
...
in \texttt{paper/evidence/browser-s2-fixed-order-T1RMZk03/}. Pandoc
