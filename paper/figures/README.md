...
Generated by `node scripts/paper-matrix.mjs` from the matrix in
`docs/COMPARISON.md`, which `npm run report` produces from the recorded adapter
traces and `npm run report:check` keeps fresh. The DOT, PDF and SVG are derived
files: edit the report generator or the scenarios, never the figure. Every cell
carries its text label, so the figure does not rely on colour.
