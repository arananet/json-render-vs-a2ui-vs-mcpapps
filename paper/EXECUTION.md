...
## Capability-matrix figure and Veritas quality checks — 26 September 2026 UTC

Added Figure 1, the capability matrix, generated by `scripts/paper-matrix.mjs`
from the matrix in `docs/COMPARISON.md` (itself generated from the recorded
adapter traces and checked by `npm run report:check`). No claim, fixture or
evidence file changed; the figure presents classifications the report already
records, captioned as classifications (C8).

`veritas.yaml` now declares the generated files (`derived:` — diagrams,
matrix, LaTeX, PDF), freezes `paper/evidence/**` with the files that change by
design excluded, and enables the numeric-traceability, presentation,
derived-freshness, frozen-integrity, pdf-inspection and venue (arXiv) checks.
`veritas build` regenerated every derivation and wrote `veritas.lock.json`.
Graphviz on this machine re-rendered the two diagrams' PDF/SVG from unchanged
DOT sources, so those bytes differ from the previous build.

Commands and results:

- `veritas build .` — diagrams, matrix, latex, pdf built; no overfull boxes.
- `node --test tests/paper/*.mjs` — 33 passed, 1 failed: the private-archive
  test requires `PAPER_BROWSER_EVIDENCE`, which was not supplied.
- `npm test` — 24 passed; `npm run typecheck` — passed.
- Veritas deterministic checks — all pass; reference-integrity reports only
  the unsupplied paths already accepted in `gate.accepted_risks`.
